...
import com.ctapweb.feature.type.POSDensityPerNoun;

public class POSDensityPerNounAE extends JCasAnnotator_ImplBase {
	...
	private static final String aeName = "POS Density per Noun Feature Extractor";
	...
	public void initialize(UimaContext aContext)
		...
		if (aContext.getConfigParameterValue(PARAM_POS_TYPE) == null) {
			ResourceInitializationException e = new ResourceInitializationException(
					"mandatory_value_missing", new Object[] { PARAM_POS_TYPE });
			logger.throwing(e);
			throw e;
		} else {
			POSType = (String) aContext.getConfigParameterValue(PARAM_POS_TYPE);
	...
	public void process(JCas aJCas) throws AnalysisEngineProcessException {
		...
		List<String> nounTagList = getNounTagList();
		...
		logger.trace(LogMarker.UIMA_MARKER, "nPOSTypes " + nPOSTypes
				+ ", nNouns " + nNouns);
		if (nNouns != 0) {
			density = (double) nPOSTypes / nNouns;
		}

		// output the feature type
		POSDensityPerNoun annotation = new POSDensityPerNoun(aJCas);
	...
	private List<String> getNounTagList() {
		String[] noun = { "NN", "NNS", "NNP", "NNPS" };
		List<String> tagList = new ArrayList<>(Arrays.asList(noun));
		return tagList;
	}
